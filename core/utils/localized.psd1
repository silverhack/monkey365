...
AzureADSelectedTenantInfo        = Tenant {0} was selected
AzureConsoleBadInputError        = Bad input received. Please type only a valid number from the [ID] column
AzureADServicePrincipalInfo      = Getting service principal {0} from Azure AD
AzureADServicePrincipalAllInfo   = Getting service principals from Azure AD
AzureADServicePrincipalPermInfo  = Getting permissions for service principal {0}
AzureADApplicationInfo           = Getting application {0} from Azure AD
AzureADApplicationAllInfo        = Getting applications from Azure AD
AzureADAppPermissionInfo         = Getting permissions for application {0}
...
M365AADInfoError                 = Unable to get Azure AD license information
...
ExpiredTokenMessage			     = Access token for {0} will expire in {1} minutes. Obtaining a new access token using refresh token
ExpiredPsSessionMessage			 = PsSession will expire in {0} minutes. Obtaining a new PsSession for {1}
