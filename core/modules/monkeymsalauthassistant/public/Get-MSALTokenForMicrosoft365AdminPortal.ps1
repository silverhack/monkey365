...
Function Get-MSALTokenForMicrosoft365AdminPortal{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MSALTokenForMicrosoft365AdminPortal
            Version     : 1.0
    ...
    Process{
        ...
        if($isPublicApp -eq $false){
            Write-Warning -Message ($Script:messages.NotAllowedToAuth -f "Microsoft 365 admin portal")
            #Remove common params
            $internal_params = Remove-MSALPublicParam -parameters $internal_params
        }
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $AzureEnvironment.OfficeAdminPortal;
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Microsoft 365 admin portal")
            ...
            #Write message
            Write-Warning -Message ($Script:messages.UnableToGetToken -f "Microsoft 365 admin portal")
            return $null
        }
    }
}
