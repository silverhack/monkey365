...
Function Get-MSALTokenForAzurePortal{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MSALTokenForAzurePortal
            Version     : 1.0
    ...
    Process{
        ...
        if($isPublicApp -eq $false){
            Write-Warning -Message ($Script:messages.NotAllowedToAuth -f "Azure Portal AAD")
            #Remove common params
            $internal_params = Remove-MSALPublicParam -parameters $internal_params
        }
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $AzurePortalWellKnownId;
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Azure AAD Portal")
