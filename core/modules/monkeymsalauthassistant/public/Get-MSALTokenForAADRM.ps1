...
Function Get-MSALTokenForAADRM{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MSALTokenForAADRM
            Version     : 1.0
    ...
    Begin{
        ...
        #Set clientId
        if($isPublicApp -or $PublicApp){
            $internal_params.Add('RedirectUri',"https://aadrm.com/adminpowershell")
        }
    }
    Process{
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $AzureEnvironment.AADRM;
    }
    End{
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Right Management Services")
