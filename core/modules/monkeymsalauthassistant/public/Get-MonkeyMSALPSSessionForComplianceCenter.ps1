...
Function Get-MonkeyMSALPSSessionForComplianceCenter {
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MonkeyMSALPSSessionForComplianceCenter
            Version     : 1.0
    ...
    Begin{
        ...
        if($isPublicApp -eq $false -or $ConfidentialApp){
            ...
            Write-Warning -Message ($Script:messages.NotAllowedToAuth -f $AzureEnvironment.ComplianceCenter)
        }
        #Set clientId
        if($null -ne $PublicApp -and $PublicApp -is [Microsoft.Identity.Client.PublicClientApplication]){
            if($PublicApp.AppConfig.ClientId -ne (Get-WellKnownAzureService -AzureService ExchangeOnlineV2)){
                ...
                if($PSEdition -eq "Desktop"){
                    $app_param.RedirectUri = (Get-MonkeyExoRedirectUri -Environment $Environment)
                }
        ...
        if($null -ne $exo_login -and $exo_login -is [Microsoft.Identity.Client.AuthenticationResult]){
            ...
            if($null -ne $Ctoken){
                $params = @{
                    ...
                    ConnectionUri = ("{0}?BasicAuthToOAuthConversion=true" -f $AzureEnvironment.ComplianceCenter)
                    Credential = $Ctoken
                ...
                try{
                    $ComplianceSession = New-PSSession @params
                }
                catch{
                    #https://github.com/PowerShell/PowerShell/issues/7567
                    #On linux systems, it seems that after receiving 302 response, no further request was sent
    ...
    End{
        if($null -ne $ComplianceSession -and $ComplianceSession -is [System.Management.Automation.Runspaces.PSSession]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Exchange Compliance Center");
            ...
            Write-Warning -Message ($Script:messages.ConnectionError -f "Exchange Compliance Center")
            return $null
        }
    }
}
