...
Function Get-MSALTokenForGraph{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MSALTokenForGraph
            Version     : 1.0
    ...
    Process{
        ...
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $AzureEnvironment.Graph;
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Graph")
            ...
            #Write message
            Write-Warning -Message ($Script:messages.UnableToGetToken -f "Graph")
            return $null
        }
    }
}
