...
Function Get-MSALTokenForTeams{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MSALTokenForTeams
            Version     : 1.0
    ...
    Begin{
        ...
        if($isPublicApp -eq $false){
            ...
            Write-Warning -Message ($Script:messages.NotAllowedToAuth -f "Microsoft Teams")
        }
    }
    Process{
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $TeamsApiWellKnownId;
    }
    End{
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Microsoft Teams")
