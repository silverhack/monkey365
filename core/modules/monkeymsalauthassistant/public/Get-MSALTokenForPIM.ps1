...
Function Get-MSALTokenForPIM{
    ...
    Process{
        ...
        $PIMWellKnownId = Get-WellKnownAzureService -AzureService MSPIM
        $isPublicApp = Confirm-IfMSALPublicApp -parameters $PSBoundParameters
        $internal_params = $PSBoundParameters
        if($isPublicApp -eq $false){
            Write-Warning -Message ($Script:messages.NotAllowedToAuth -f "Privileged Identity Management")
            #Remove common params
            $internal_params = Remove-MSALPublicParam -parameters $internal_params
        }
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $PIMWellKnownId;
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.SuccessfullyConnectedTo -f "Azure AD Privileged Identity Management")
            ...
            #Write message
            Write-Warning -Message ($Script:messages.UnableToGetToken -f "Azure AD Privileged Identity Management")
            return $null
        }
    }
}
