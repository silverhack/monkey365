...
Function Get-MSALTokenForPowerBI{
    ...
    Process{
        ...
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $AzureEnvironment.PowerBI;
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.SuccessfullyConnectedTo -f "Microsoft Power BI")
