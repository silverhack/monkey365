...
Function Get-MSALTokenForPowerBI{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MSALTokenForPowerBI
            Version     : 1.0
    ...
    Process{
        ...
        if($PSBoundParameters.ContainsKey('Endpoint') -and $PSBoundParameters['Endpoint']){
            $resource = $PSBoundParameters['Endpoint']
        }
        else{
            $resource = $AzureEnvironment.PowerBI
        }
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $resource;
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Microsoft Power BI")
            ...
            #Write message
            Write-Warning -Message ($Script:messages.UnableToGetToken -f "Microsoft Power BI")
            return $null
        }
    }
}
