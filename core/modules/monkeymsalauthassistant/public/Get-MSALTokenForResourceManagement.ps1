...
Function Get-MSALTokenForResourceManagement{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MSALTokenForResourceManagement
            Version     : 1.0
    ...
    Process{
        ...
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $AzureEnvironment.ResourceManager;
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Resource Management")
            ...
            #Write message
            Write-Warning -Message ($Script:messages.UnableToGetToken -f "Resource Management")
            return $null
        }
    }
}
