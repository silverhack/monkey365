...
Function Get-MSALTokenForIntune{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MSALTokenForIntune
            Version     : 1.0
    ...
    Begin{
        ...
        if($null -ne $PublicApp -and $PublicApp -is [Microsoft.Identity.Client.PublicClientApplication]){
            if($PublicApp.AppConfig.ClientId -ne (Get-WellKnownAzureService -AzureService Intune)){
                #Add clientId and RedirectUri
                $app_param = @{}
                $app_param.ClientId = (Get-WellKnownAzureService -AzureService Intune)
                if($PSBoundParameters.ContainsKey('TenantId')){
                    $app_param.TenantId = $TenantId
                }
                $intune_app = New-MonkeyMsalApplication @app_param
                [ref]$null = $internal_params.Remove('PublicApp')
                [ref]$null = $internal_params.Add('PublicApp',$intune_app)
            }
        }
    }
    Process{
        $access_token = Get-MonkeyMSALToken @internal_params -Resource $AzureEnvironment.GraphV2;
    }
    End{
        if($null -ne $access_token -and $access_token -is [Microsoft.Identity.Client.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.TokenAcquiredInfoMessage -f "Endpoint Manager")
            ...
            #Write message
            Write-Warning -Message ($Script:messages.UnableToGetToken -f "Endpoint Manager")
            return $null
        }
    }
}
