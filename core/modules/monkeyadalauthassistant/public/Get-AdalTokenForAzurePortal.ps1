...
Function Get-AdalTokenForAzurePortal{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-AdalTokenForAzurePortal
            Version     : 1.0
    ...
    $AzurePortalWellKnownId = Get-WellKnownAzureService -AzureService AzurePortal
    $isPublicApp = Confirm-IfPublicApp -parameters $PSBoundParameters
    if($isPublicApp -eq $false){
        Write-Warning -Message ($Script:messages.NotAllowedToAuth -f $AzurePortalWellKnownId)
        #Remove common params
        $internal_params = Remove-PublicParams -parameters $internal_params
    }
    $access_token = Get-MonkeyAdalToken @internal_params -Resource $AzurePortalWellKnownId;
    if($null -ne $access_token -and $access_token -is [Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationResult]){
        #Write message
        $msg = @{
            MessageData = ($Script:messages.SuccessfullyConnectedTo -f "Azure AAD Portal")
        ...
        #Write message
        Write-Warning -Message ($Script:messages.UnableToGetToken -f "Azure AAD Portal")
        return $null
    }
}
