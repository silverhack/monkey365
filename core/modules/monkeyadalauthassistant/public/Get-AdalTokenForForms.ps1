...
Function Get-AdalTokenForForms {
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-AdalTokenForForms
            Version     : 1.0
    ...
    Begin{
        ...
        #Set clientId
        if($isPublicApp){
            if($PSBoundParameters.ContainsKey('ClientId') -eq $false){
                $PSBoundParameters.Add('ClientId',(Get-WellKnownAzureService -AzureService GlobalPowerShell))
            }
        }
    }
    Process{
        $forms_login = Get-MonkeyAdalToken @auth_params -Resource $FormsWellKnownId
    }
    End{
        if($null -ne $forms_login -and $forms_login -is [Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationResult]){
            #Write message
            $msg = @{
