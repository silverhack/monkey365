...
Function Get-AdalTokenForGraphV2{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-AdalTokenForGraphV2
            Version     : 1.0
    ...
    if($isPublicApp -eq $false){
        ...
        $internal_params = Remove-PublicParams -parameters $PSBoundPinternal_paramsarameters
    }
    ...
    if($null -ne $access_token -and $access_token -is [Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationResult]){
        #Write message
        $msg = @{
            MessageData = ($Script:messages.SuccessfullyConnectedTo -f "Microsoft Graph")
        ...
        #Write message
        Write-Warning -Message ($Script:messages.UnableToGetToken -f "Microsoft Graph")
        return $null
    }
}
