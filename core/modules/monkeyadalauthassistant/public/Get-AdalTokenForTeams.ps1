...
Function Get-AdalTokenForTeams{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-AdalTokenForTeams
            Version     : 1.0
    ...
    Begin{
        ...
        $TeamsApiWellKnownId = Get-WellKnownAzureService -AzureService TeamsAdminApi
        ...
        #Set clientId
        if($isPublicApp){
            if($internal_params.ContainsKey('ClientId') -eq $false){
                $internal_params.Add('ClientId',(Get-WellKnownAzureService -AzureService GlobalPowerShell))
            }
        }
    }
    Process{
        $access_token = Get-MonkeyAdalToken @internal_params -Resource $TeamsApiWellKnownId;
    }
    End{
        if($null -ne $access_token -and $access_token -is [Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.SuccessfullyConnectedTo -f "Microsoft Teams")
            ...
            #Write message
            Write-Warning -Message ($Script:messages.UnableToGetToken -f "Microsoft Teams")
            return $null
        }
    }
}
