...
Function Get-AdalTokenForIntune{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-AdalTokenForIntune
            Version     : 1.0
    ...
    Begin{
        ...
        #Set clientId
        if($isPublicApp){
            $internal_params.Add('ClientId',(Get-WellKnownAzureService -AzureService Intune))
            $internal_params.Add('RedirectUri',"urn:ietf:wg:oauth:2.0:oob")
        }
    }
    Process{
        $access_token = Get-MonkeyAdalToken @internal_params -Resource $AzureEnvironment.GraphV2;
    }
    End{
