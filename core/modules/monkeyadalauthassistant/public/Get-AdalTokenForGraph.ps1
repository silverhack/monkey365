...
Function Get-AdalTokenForGraph{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-AdalTokenForGraph
            Version     : 1.0
    ...
    $access_token = Get-MonkeyAdalToken @internal_params -Resource $AzureEnvironment.Graph;
    if($null -ne $access_token -and $access_token -is [Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationResult]){
        #Write message
        $msg = @{
            MessageData = ($Script:messages.SuccessfullyConnectedTo -f "Graph")
