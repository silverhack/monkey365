...
Function Get-AdalTokenForAADRM{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-AdalTokenForAADRM
            Version     : 1.0
    ...
    Begin{
        ...
        #Set clientId
        if($isPublicApp){
            $internal_params.Add('ClientId',(Get-WellKnownAzureService -AzureService AADRM))
            $internal_params.Add('RedirectUri',"https://aadrm.com/adminpowershell")
        }
    }
    Process{
        $access_token = Get-MonkeyAdalToken @internal_params -Resource $AzureEnvironment.AADRM;
    }
    End{
        if($null -ne $access_token -and $access_token -is [Microsoft.IdentityModel.Clients.ActiveDirectory.AuthenticationResult]){
            #Write message
            $msg = @{
                MessageData = ($Script:messages.SuccessfullyConnectedTo -f "Right Management Services")
            ...
            #Write message
            Write-Warning -Message ($Script:messages.UnableToGetToken -f "Right Management Services")
            return $null
        }
    }
}
