...
Function Initialize-HtmlData_old{
    <#
        .SYNOPSIS
    ...
    Process{
        ...
        If($null -ne $InputObject.affectedResources){
            ...
            if($null -ne $InputObject.actions.objectData.PsObject.Properties.Item('limit') -and $null -ne $InputObject.actions.objectData.limit){
                ...
                if($integer){
                    $elements = $InputObject.affectedResources | Select-Object -First $InputObject.actions.objectData.limit
                }
                else{
                ...
                $elements = $InputObject.affectedResources
            }
            If($null -ne $InputObject.translate){
                ForEach($element in $elements){
                    ...
                    ForEach($prop in $InputObject.translate.PsObject.Properties){
                        try{
                            ...
                            Write-Warning ("Error in {0}" -f $issue.idSuffix)
                ...
                Write-Verbose ("Empty translate properties found on {0}" -f $InputObject.idSuffix)
                #convert objects
