...
<#
        .SYNOPSIS
		Collector to get information about Teams application permission policy
	...
	process {
		if ($null -ne $access_token) {
			$msg = @{
				MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Microsoft Teams: application policies",$O365Object.TenantID);
			...
			$app_policies = Get-MonkeyTeamsObject @params
		}
	}
	end {
		if ($app_policies) {
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Microsoft Teams: Skype application policies",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
