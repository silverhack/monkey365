...
<#
        .SYNOPSIS
		Collector to get information about Teams Tenant federation settings (Skype)
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ruleSuffixes = @(
				"o365_teams_skype_federation_settings"
			);
			dependsOn = @(
	...
	process {
		if ($null -ne $access_token) {
			$msg = @{
				MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Microsoft Teams: Skype tenant federation settings",$O365Object.TenantID);
		...
		if ($fed_settings) {
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Microsoft Teams: Skype tenant federation settings",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
