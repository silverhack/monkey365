...
<#
        .SYNOPSIS
		Collector to get information about AADRM doNotTrack
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			collectorName = "Get-MonkeyAADRMDoNotTrackUserGroup";
			ApiType = $null;
			description = "Collector to get information about AADRM doNotTrack";
			Group = @(
				"Purview";
				"ExchangeOnline"
			);
	...
	process {
		if ($requestHeader -and $url) {
			$msg = @{
				MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Office 365 Rights Management: DoNotTrack feature",$O365Object.TenantID);
