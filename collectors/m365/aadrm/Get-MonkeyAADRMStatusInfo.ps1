...
<#
        .SYNOPSIS
		Collector to get information about AADRM status
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			collectorName = "Get-MonkeyAADRMStatusInfo";
			ApiType = $null;
			description = "Collector to get information about AADRM status";
			Group = @(
				"Purview";
				"ExchangeOnline"
			);
	...
	process {
		if ($requestHeader -and $url) {
			$msg = @{
				MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Office 365 Rights Management: Status",$O365Object.TenantID);
