...
<#
        .SYNOPSIS
		Collector to get information about superuser in AADRM
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			collectorName = "Get-MonkeyAADRMServiceSuperUser";
			ApiType = $null;
			description = "Collector to get information about superuser in AADRM";
			Group = @(
				"Purview";
				"ExchangeOnline"
			);
	...
	process {
		if ($requestHeader -and $url) {
			$msg = @{
				MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Office 365 Rights Management: Service Super user state",$O365Object.TenantID);
