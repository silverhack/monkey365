...
<#
        .SYNOPSIS
		Collector to get information about Teams guest meeting configuration
	...
	process {
		if ($null -ne $access_token) {
			$msg = @{
				MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Microsoft Teams: Guest meeting configuration",$O365Object.TenantID);
		...
		if ($guest_meet_conf) {
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Microsoft Teams: Guest meeting configuration",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
