...
<#
        .SYNOPSIS
		Plugin to get subscription policies in Azure
	...
	Begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			PluginName = "Get-MonkeySubscriptionPolicy";
			ApiType = "resourceManagement";
			Title = "Plugin to get subscription policies in Azure";
			Group = @("Subscription","General");
			Tags = @{
				"enabled" = $true
			};
	...
	Process {
		$msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $pluginId,"subscription policies",$O365Object.current_subscription.displayName);
