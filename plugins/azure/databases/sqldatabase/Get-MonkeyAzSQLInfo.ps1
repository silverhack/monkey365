...
<#
        .SYNOPSIS
		Plugin to get information about SQL Databases from Azure
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			PluginName = "Get-MonkeyAzSQLInfo";
			ApiType = "resourceManagement";
			Title = "Plugin to get information about SQL Databases from Azure";
			Group = @("Databases");
			Tags = @{
				"enabled" = $true
			};
	...
	process {
		...
		if ($DatabaseServers) {
            $AllServers = $DatabaseServers | Get-MonkeyAzSQlServer -APIVersion $AzureSQLConfig.api_version
		}
	}
	end {
		if ($AllServers) {
