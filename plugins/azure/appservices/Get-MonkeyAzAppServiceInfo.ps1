...
<#
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			PluginName = "Get-MonkeyAzAppServiceInfo";
	...
	process {
		...
		if ($app_services) {
            foreach($app in $app_services){
                $app_data = $app | Get-MonkeyAzAppService -APIVersion $config.api_version
                if($app_data){
                    [void]$all_apps.Add($app_data);
                }
            }
		}
	}
