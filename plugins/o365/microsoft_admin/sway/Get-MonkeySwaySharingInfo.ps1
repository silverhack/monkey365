...
<#
        .SYNOPSIS
		Plugin to get sharing information from Microsoft Sway
	...
	Begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			PluginName = "Get-MonkeySwaySharingInfo";
			ApiType = $null;
			Title = "Plugin to get sharing information from Microsoft Sway";
			Group = @("Microsoft365");
			Tags = @{
				"enabled" = $true
			};
	...
	process {
		$msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $pluginId,"Microsoft Sway. Sharing Settings",$O365Object.TenantID);
