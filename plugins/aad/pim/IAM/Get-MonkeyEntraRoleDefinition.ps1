...
<#
        .SYNOPSIS
		Plugin to get information about role definition from PIM
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			PluginName = "Get-MonkeyEntraRoleDefinition";
			ApiType = "PIM";
			Title = "Plugin to get information about role definition from PIM";
			Group = @("AzureAD");
			Tags = @{
				"enabled" = $true
			};
	...
	process {
        $msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $pluginId,"Azure AD Privileged Identity Management",$O365Object.TenantID);
		...
		if ($null -ne $role_definition) {
			$role_definition.PSObject.TypeNames.Insert(0,'Monkey365.AzureAD.PIM.RoleDefinition')
			[pscustomobject]$obj = @{
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Azure AD Privileged Identity Management",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
				Verbose = $O365Object.Verbose;
				Tags = @('AzureADPIMRoleAssignmentEmptyResponse')
			}
			Write-Verbose @msg
		}
        if ($all_active_ra.Count -gt 0) {
			$all_active_ra.PSObject.TypeNames.Insert(0,'Monkey365.AzureAD.PIM.RoleDefinition')
			[pscustomobject]$obj = @{
				Data = $all_active_ra;
				Metadata = $monkey_metadata;
			}
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Azure AD Privileged Identity Management active assignments",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
				Verbose = $O365Object.Verbose;
				Tags = @('AzureADPIMActiveRoleAssignmentEmptyResponse')
			}
			Write-Verbose @msg
		}
        if ($all_eligible_ra.Count -gt 0) {
			$all_eligible_ra.PSObject.TypeNames.Insert(0,'Monkey365.AzureAD.PIM.RoleDefinition')
			[pscustomobject]$obj = @{
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Azure AD Privileged Identity Management eligible assignments",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
				Verbose = $O365Object.Verbose;
				Tags = @('AzureADPIMEligibleRoleAssignmentEmptyResponse')
			}
			Write-Verbose @msg
		}
	}
