...
<#
        .SYNOPSIS
		Plugin to get Azure AD connect information
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			PluginName = "Get-MonkeyADConnectInfo";
			ApiType = "AzureADPortal";
			Title = "Plugin to get Azure AD connect information";
			Group = @("AzureADPortal");
			Tags = @{
				"enabled" = $true
			};
	...
	process {
		$msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $pluginId,"Azure AD Connect Info",$O365Object.TenantID);
		...
		if ($null -ne $ad_connect_dict) {
			...
			$adObject.PSObject.TypeNames.Insert(0,'Monkey365.AzureAD.connect.info')
			[pscustomobject]$obj = @{
				Data = $adObject;
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Azure AD Connect Info",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
