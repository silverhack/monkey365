...
<#
        .SYNOPSIS
		Plugin to get app registration info from Azure AD
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			PluginName = "Get-MonkeyAADApplication";
			ApiType = "MSGraph";
			Title = "Plugin to get app registration info from Azure AD";
			Group = @("AzureAD");
			Tags = @{
				"enabled" = $true
			};
	...
	process {
		$msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $pluginId,"Azure AD applications",$O365Object.TenantID);
			callStack = (Get-PSCallStack | Select-Object -First 1);
			logLevel = 'info';
			InformationAction = $O365Object.InformationAction;
			Tags = @('AzureADMSGraphApplication');
		}
		...
		if ($all_applications) {
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Azure AD applications",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
				Verbose = $O365Object.Verbose;
		...
		if ($app_perms) {
			$app_perms.PSObject.TypeNames.Insert(0,'Monkey365.AzureAD.application_permissions')
			[pscustomobject]$obj = @{
				Data = $app_perms;
				Metadata = $monkey_metadata;
			}
			...
			$msg = @{
				MessageData = ($message.MonkeyEmptyResponseMessage -f "Azure AD application's permissions",$O365Object.TenantID);
				callStack = (Get-PSCallStack | Select-Object -First 1);
				logLevel = "verbose";
				InformationAction = $O365Object.InformationAction;
				Verbose = $O365Object.Verbose;
