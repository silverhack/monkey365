...
Function Get-NewFilter{
    ...
    Process{
        ...
        If($LeftItem -match '\{\$cutoffDateInDays\((\d+)\)\}'){
            $days = $Matches[1]
            If($null -ne $days){
                $LeftItem = (Get-Date).AddDays(-$days)
            }
        }
        If($RightItem -match '\{\$cutoffDateInDays\((\d+)\)\}'){
            $days = $Matches[1]
            If($null -ne $days){
                $RightItem = (Get-Date).AddDays(-$days)
            }
        }
        ...
        ElseIf($RightCastValue -is [string]){
            ...
            ElseIf($LeftItem.Contains('@odata.type')){
                ...
                Else{
                    If($leftQuery.Length -eq 0){
                        $tmp_filter = ('$_."{0}" -{1} {2}' -f '@odata.type', $Operator, $rightCondition)
                    }
                    Else{
