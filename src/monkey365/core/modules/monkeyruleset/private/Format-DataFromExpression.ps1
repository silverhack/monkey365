                        ...
                        If($_expressions | Test-IsPsObject){
                            ...
                            Else{
                                ForEach($element in $_expressions.PsObject.Properties){
                                    ...
                                    $property = [System.Text.StringBuilder]::new()
                                    If($element.Name.Trim().ToString().Contains('@')){
                                        If($element.Name.Trim().ToString().Contains('@odata.type')){
                                            #First remove odata.type
                                            $_oldProperty = $element.Name.Trim().ToString() -replace '\.?["'']?@odata\.type["'']?',""
                                            If($_oldProperty.Contains('.')){
                                                $_parts = [System.Text.StringBuilder]::new()
                                                ForEach($part in $_oldProperty.Split('.')){
                                                    [void]$_parts.Append($part);
                                                    [void]$_parts.Append('.')
                                                }
                                                $_newProperty = ('{0}"{1}"' -f $_parts.ToString(),'@odata.type')
                                                #Add property
                                                [void]$property.Append($_newProperty);
                                            }
                                            Else{
                                                If($_oldProperty.Length -eq 0){
                                                    $_newProperty = ('"{0}"' -f '@odata.type')
                                                }
                                                Else{
                                                    $_newProperty = ('{0}."{1}"' -f $_oldProperty,'@odata.type')
                                                }
                                                #Add property
                                                [void]$property.Append($_newProperty);
                                            }
