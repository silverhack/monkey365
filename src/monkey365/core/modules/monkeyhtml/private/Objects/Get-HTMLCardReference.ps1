...
Function Get-HTMLCardReference{
    <#
        .SYNOPSIS
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-HTMLCardReference
            Version     : 1.0
    ...
    Param (
        ...
        [parameter(Mandatory= $True, HelpMessage= "References")]
        [Object]$Content,
    ...
    Process{
        ...
        If($null -ne $PSBoundParameters['Content'] -and $PSBoundParameters['Content']){
            Foreach($reference in @($PSBoundParameters['Content'])){
                #Create a object
                $attributes =  @{
                    Class = "external-link";
                    target = "_blank";
                ...
                #Create element
                $Link = New-HtmlTag @AProperties
                #Add link
                [void]$textObj.AppendChild($Link);
