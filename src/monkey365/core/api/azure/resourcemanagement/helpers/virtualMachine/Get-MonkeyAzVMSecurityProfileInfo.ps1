    ...
    Process{
        try{
            ...
            If($null -ne $securityProfile){
                $InputObject.securityProfile.encryptionAtHost = $securityProfile | Select-Object -ExpandProperty encryptionAtHost -ErrorAction Ignore;
                ...
                If($null -ne $securityType){
                    $InputObject.securityProfile.securityType = $securityType;
                }
