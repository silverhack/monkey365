...
Function Get-MonkeyAzMySQlServer {
    <#
        .SYNOPSIS
		Get MySQl server from Azure
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MonkeyAzMySQlServer
            Version     : 1.0
    ...
    Process{
        try{
            ...
            If($dbServer){
                ...
                $new_dbServer.networking.privateDNS = $dbServer.properties.network | Select-Object -ExpandProperty privateDnsZoneResourceId -ErrorAction Ignore;
                #Get Configuration
                $new_dbServer.configuration = $new_dbServer | Get-MonkeyAzOSSQlConfig -APIVersion $APIVersion
                If($new_dbServer.configuration){
                    #Get Tls version
                    $new_dbServer.networking.minimumTlsVersion = $new_dbServer.configuration.Where({$_.parameterName -eq 'tls_version'}) | Select-Object -ExpandProperty parameterValue
                }
                #Get databases
                $new_dbServer.databases = $dbServer | Get-MonkeyAzMySQlDatabase -APIVersion $APIVersion
                ...
                #######Get Entra ID only admin ########
                $p = @{
					InputObject = $new_dbServer;
                    APIVersion = $APIVersion;
                    EntraIDOnlyForMysql = $True;
                    InformationAction = $O365Object.InformationAction;
                    Verbose = $O365Object.verbose;
                    Debug = $O365Object.debug;
				}
                ...
                If($onlyEid){
                    ...
                    If($null -ne $props){
                        $new_dbServer.sqlAd.entraIdOnlyAuthentication.enabled = If($props.currentValue.ToLower() -eq 'off'){$false}Else{$True}
                    }
                    Else{
