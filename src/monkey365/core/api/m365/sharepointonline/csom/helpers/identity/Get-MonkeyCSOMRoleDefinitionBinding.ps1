...
Function Get-MonkeyCSOMRoleDefinitionBinding{
    <#
        .SYNOPSIS
        Get role definition bindings from SharePoint Online
        ...
        .NOTES
	        Author		: Juan Garrido
            Twitter		: @tr1ana
            File Name	: Get-MonkeyCSOMRoleDefinitionBinding
            Version     : 1.0
    ...
    Param (
        ...
        [parameter(Mandatory=$False, HelpMessage="Include Member property")]
    ...
    Process{
        try{
            ...
            If($PSBoundParameters.ContainsKey('IncludeMember') -and $PSBoundParameters['IncludeMember'].IsPresent){
                [void]$newPsboundParams.Add('Properties',("Member","RoleDefinitionBindings"))
            }
            Else{
                [void]$newPsboundParams.Add('Properties',"RoleDefinitionBindings")
            }
            Get-MonkeyCSOMProperty @newPsboundParams
        }
        Catch{
            $msg = @{
                MessageData = ("Unable to get role definition binding");
                callStack = (Get-PSCallStack | Select-Object -First 1);
                logLevel = 'Verbose';
                InformationAction = $O365Object.InformationAction;
                Verbose = $O365Object.verbose;
                Tags = @('MonkeyCSOMRoleDefinitionError');
            }
            Write-Verbose @msg
            $msg = @{
                MessageData = $_;
