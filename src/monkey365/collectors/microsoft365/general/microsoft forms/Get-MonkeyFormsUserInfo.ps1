...
<#
	...
	process {
		...
		if ($null -ne $O365Object.auth_tokens.Forms) {
			$methods = $O365Object.auth_tokens.Forms | Get-Member | Where-Object {$_.MemberType -eq 'Method'} | Select-Object -ExpandProperty Name
            #Get Authorization Header
            If($null -ne $methods -and $methods.Contains('CreateAuthorizationHeader')){
                $AuthHeader = $O365Object.auth_tokens.Forms.CreateAuthorizationHeader()
            }
            Else{
                $AuthHeader = ("Bearer {0}" -f $O365Object.auth_tokens.Forms.AccessToken)
            }
            $authHeader = @{
