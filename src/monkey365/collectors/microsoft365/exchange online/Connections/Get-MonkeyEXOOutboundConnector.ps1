...
<#
        .SYNOPSIS
		Collector to get information about outbound connector in Exchange Online
	...
	begin {
		...
		$monkey_metadata = @{
			...
			ResourceType = $null;
			resourceName = $null;
			collectorName = "Get-MonkeyEXOOutboundConnector";
			ApiType = "ExoApi";
            objectType = 'ExchangeOutboundConnector';
            immutableProperties = @(
                'DistinguishedName',
                'ExchangeObjectId.Guid'
            );
			description = "Collector to get information about outbound connector in Exchange Online";
			Group = @(
				"ExchangeOnline"
			);
			Tags = @(
	...
	process {
		$msg = @{
			MessageData = ($message.MonkeyGenericTaskMessage -f $collectorId,"Exchange Online outbound connector",$O365Object.TenantID);
		...
		$p = @{
			...
			Command = 'Get-OutboundConnector';
			Method = "POST";
			InformationAction = $O365Object.InformationAction;
			Verbose = $O365Object.Verbose;
			Debug = $O365Object.Debug;
		}
		$exo_outbound_connector = Get-PSExoAdminApiObject @p
	}
	end {
